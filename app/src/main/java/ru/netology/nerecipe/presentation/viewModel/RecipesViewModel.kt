...
import ru.netology.nerecipe.data.impl.CategoryRepositoryImpl
import ru.netology.nerecipe.data.impl.RecipeRepositoryImpl
import ru.netology.nerecipe.data.impl.StepsRepositoryImpl
import ru.netology.nerecipe.data.db.AppDb
...
class RecipesViewModel(private val inApplication: Application) :
    ...
    private val categoriesRepo: CategoryRepository =
        CategoryRepositoryImpl(AppDb.getInstance(inApplication).categoryDao)
    val catData by categoriesRepo::data
    fun saveCategory(category: Category) = categoriesRepo.save(category)
    fun getCatNameId(id: Long) = categoriesRepo.getName(id)

    // Recipes data section
    private val recipesRepo: RecipeRepository =
        RecipeRepositoryImpl(AppDb.getInstance(inApplication).recipeDao)
    ...
    private val recStepsRepo: RecipeStepsRepository =
        StepsRepositoryImpl(AppDb.getInstance(inApplication).stepDao)
    val stepsFilteredData by recStepsRepo::dataFiltered
    val stepsAllData by recStepsRepo::dataAll
    val editedStepsList: MutableList<RecipeStage> = mutableListOf()
    private var editStep: RecipeStage? = null
    var stepUri: Uri? = null
    fun saveStep(step: RecipeStage) = recStepsRepo.save(step)
    fun removeStep(step: RecipeStage) = recStepsRepo.remove(step.id)
    fun clearEditedStep() {
        editStep = null
    }
    ...
    val chooseThePicture = SingleLiveEvent<RecipeStage?>()


    override fun onChoosePictureClicked(step: RecipeStage) {
    ...
    override fun editStep(step: RecipeStage) {
        editStep = step
        isNewStep = false
    ...
    override fun updateRepoWithNewListFromTo(
        ...
        val listOfLists: MutableList<List<RecipeStage>> = mutableListOf()
    ...
    override fun updateStepsRepoWithListFromTo(list: List<RecipeStage>, dragFrom: Int, dragTo: Int) {
        ...
        val updatedList = mutableListOf<RecipeStage>()
    ...
    fun addNewEditedStep() {
        ...
        val step = RecipeStage(NEW_ITEM_ID, recId, "")
        isNewStep = true
    ...
    override fun deleteEditedStep(step: RecipeStage) {
    ...
    fun initCategories() {
        ...
        saveCategory(Category(NEW_ITEM_ID, "Европейская"))
        saveCategory(Category(NEW_ITEM_ID, "Азиатская"))
        saveCategory(Category(NEW_ITEM_ID, "Паназиатская"))
        saveCategory(Category(NEW_ITEM_ID, "Восточная"))
        saveCategory(Category(NEW_ITEM_ID, "Американская"))
        saveCategory(Category(NEW_ITEM_ID, "Русская"))
        saveCategory(Category(NEW_ITEM_ID, "Средиземноморская"))
    }

    fun getEditedStep(): RecipeStage? {
        return editStep
    }

    fun onSaveEditedStep(newStep: RecipeStage) {
        saveStep(newStep)
        editStep = null
    }
    ...
    fun getStepById(stepId: Long): RecipeStage {
